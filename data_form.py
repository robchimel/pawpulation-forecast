...
with st.form("intake_form"):
    FORM_DATA[1] = st.text_input("What is their name?", "FIDO")

    FORM_DATA[2] = st.selectbox("What is their type?",("Dog", "Cat"),"Dog")

    FORM_DATA[3] = st.text_input("What is their breed?", "GERM SHEPHERD")

    FORM_DATA[4] = st.text_input("What is their color?", "TAN/BLACK")

    FORM_DATA[5] = st.selectbox("What is their sex?", ("Female", "Male", "Unknown"), index="Male")

    FORM_DATA[6] = st.selectbox("What is their size?", ("Small", "Medium", "Kittn", "Large", "Toy", "Puppy", "X-LRG", "Unknown"), index="Large")

    FORM_DATA[7] = st.date_input("When is their birthday?", datetime.date(2022, 5, 11))

    FORM_DATA[8] = st.text_input("What is their kennel number?", "DA31")

    FORM_DATA[9] = st.date_input("When did they go through intake processing?", datetime.date(2024, 1, 21))

    FORM_DATA[10] = st.selectbox("What is their intake type?",
    ("Stray", "Owner Surrender", "Confiscate", "Quarantine", "Adoption Return", "Transfer", "Born Here", "Unknown"), index="Owner Surrender")
    ...
    ("Field", "Over the counter", "Comm cat", "Fld_arrest", "phone", "vet_hosp", "fld_stray", "fld_hosptl", "priv_shelt", "born_here",
     ...
     "fld_livstk", "over the counter_coronr", "over the counter_evict", "mom os", "over the counter_emerge", "Unknown"),index="Over the counter")

    FORM_DATA[12] = st.selectbox(
    "What is their condition",
    ("Healthy", "Treatable/Rehab", "Untreatable", "Treatable/Manageable","Unknown"),index="Healthy")
    ...
    ("Santa Rosa", "County", "Windsor", "Out of County", "Rohnert Park", "Healdsburg", "Sonoma", "Petaluma", "Cloverdale", "sebastopol",
     "tribal resv", "Cotati", "Unknown"),index="County")
